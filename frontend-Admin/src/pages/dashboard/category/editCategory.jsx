...
const EditCategory = ({ category, onCategoryUpdated, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8081/categories/${categoryData.id}`,
        categoryData,
        {
          headers: {
      ...
      console.log('Danh mục đã được cập nhật.');
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg relative w-1/2">
        <button
          onClick={onClose}
        ...
        <div className="mb-4">
          <label htmlFor="categoryId" className="block text-sm font-medium text-gray-700">ID</label>
          <input
            id="categoryId"
            type="text"
        ...
        <div className="flex justify-end">
          <button type="submit" className="bg-blue-500 text-white px-6 py-2 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-opacity-50">
            Cập nhật
          </button>
        </div>
      </form>
